module.exports = (app) => {
    ...
    router.post('/', async (req, res) => {
      try {
        const { materialType, materialId, photoData } = req.body;
        if (!materialId || !photoData || !materialType)
          return res.status(400).json({ error: 'Missing materialId, materialType, or photoData' });
